...
- OpenID-Connect-Anmeldung über Authentik mit Authorization Code Flow, PKCE,
  `state` und `nonce`.
- Stille Prüfung einer vorhandenen Authentik-Sitzung und direkte Weiterleitung
  ohne erneute Login-Anzeige.
- Eigener Profil-Tab **Anmeldekonten** zum Anzeigen und sicheren Entfernen der
  Authentik-Verknüpfung.
- Konfigurationsschalter zum Deaktivieren der lokalen Anmeldung und der
  automatischen Benutzeranlage.
- Persistente Zuordnung externer Identitäten über Issuer und Subject.

### Changed

- Passwortfelder im Authentik-Trenn-Dialog verwenden den gemeinsamen
  `FormGenerator`.
- Lokale Login-, Registrierungs- und Passwortfunktionen werden bei
  deaktivierter lokaler Authentifizierung auch serverseitig blockiert.
- Ein bewusster Logout löst im aktuellen Browser-Tab keinen unmittelbaren
  automatischen SSO-Login aus.

### Security

- Bestehende Konten werden nur über bestätigte E-Mail-Claims automatisch mit
  Authentik verknüpft.
- Das Entfernen einer Authentik-Verknüpfung ist gegen eine versehentliche
  Aussperrung abgesichert.
