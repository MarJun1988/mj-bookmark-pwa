...
export const Query = {
  myOidcIdentities: async (_: unknown, __: unknown, ctx: Context) => {
    requireAuth(ctx);
    return ctx.prisma.oidcIdentity.findMany({
  ...
  exportBookmarks: async (_parent: unknown, _args: unknown, ctx: Context) => {
    requireAuth(ctx);
