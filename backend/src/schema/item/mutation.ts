...
export const Mutation = {
  createItem: async (_: unknown, args: any, ctx: Context) => {
    ...
    const item: Item = await ctx.prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      const order = args && args.input.order ? args.input.order : (lastItem?.order ?? 0) + 1;
  ...
  refreshAllFavicons: async (_: unknown, { force }: { force: boolean }, ctx: Context) => {
    return refreshAllFavicons({ force }, ctx);
  },
