...
import { getUserDisplayName, hashPassword, isValidEmail } from "../../utils/utils.js";
...
export const Mutation = {
  ...
  refreshToken: async (_: unknown, __: unknown, ctx: Context) => {
    ...
    const { token: newRefreshToken, hash: newHash, expiresAt } = signRefreshToken();
  ...
  deleteAccount: async (_: unknown, args: { email: string; password: string }, ctx: Context) => {
    // 🔐 Auth Pflicht
    requireAuth(ctx);
