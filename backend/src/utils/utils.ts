...
export const buildPrismaWhere = (
  ...
  for (const [field, filter] of Object.entries(filters)) {
    ...
    if (filter.matchMode === "in" && Array.isArray(filter.value) && filter.value.length === 0) {
      continue;
    }
    ...
    const path: string[] =
      Array.isArray(config.path) && config.path.length > 0 ? config.path : [field];
...
async function resolveTags(prisma: Context["prisma"], tags: string[]) {
  ...
  const normalizedName = [...new Set(tags.map(normalizeTagName).filter(Boolean))];
...
export async function setItemTags(prisma: Context["prisma"], itemId: string, tags: string[]) {
  const tagRecords: {
    id: string;
    name: string;
    createdAt: Date;
