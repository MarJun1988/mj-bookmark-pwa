...
export function normalizeUrl(input: string): string {
  ...
  // Prüft auf http:// oder https://
  if (/^https?:\/\//i.test(trimmed)) {
    return trimmed;
  }

  // Standard: http:// ergänzen
  return `http://${trimmed}`;
}
...
export async function fetchLinkMeta(rawUrl: string): Promise<LinkMetaResult> {
  console.log(`🔎 Fetching link meta: ${rawUrl}`);

  const url = normalizeUrl(rawUrl);
  console.log(`🔎 Fetching link meta: ${url}`);

  try {
    const res = await safeFetch(normalizeUrl(url), {
      ...dispatcherOptions(),
      redirect: "follow",
    ...
    const title =
      $('meta[property="og:title"]').attr("content") || $("title").text() || new URL(url).hostname;
