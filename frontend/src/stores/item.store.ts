...
export const useItemStore = defineStore("itemStore", () => {
  ...
  const fetchOnlyItem = async (id: string | RouteParamValue[]) => {
    try {
      ...
      item.value.updatedAt = data.item.updatedAt ? new Date(data.item.updatedAt) : undefined;
    } catch (err) {
  ...
  const createItem = async (input?: Partial<Item>) => {
    ...
    try {
      const vars: Partial<Item> = {
        ...
        // tagIds: input?.tags as string[] ?? item.value.tagIds,
  ...
  // ✅ UPDATE
  const { mutate: updateMutateItem, loading: updatingLoading } =
    useMutation(MUTATIONEN_UPDATE_ITEM);
  ...
  // ✅ DELETE
  const { mutate: deleteMutateItem, loading: deletingLoading } =
    useMutation(MUTATIONEN_DELETE_ITEM);
   ...
   */
  const routeToNewItem = (tabId?: string, groupId?: string | null): RouteLocationRaw => {
    return {
