...
import type { DashboardMode, Group, Item, ItemTypeValue, Tab, TabView } from "@/utils/interfaces";
import type { SearchResult, User } from "@/utils/interfaces.ts";
import { type RouteLocationNormalizedLoaded, type Router, useRoute } from "vue-router";
            ...
            for (const item of group.items ?? []) {
              ...
              if (!match) {
                match =
                  item.tags?.some(
                    (tag) =>
                      tag.slug?.toLowerCase().includes(q) || tag.name?.toLowerCase().includes(q),
                  ) ?? false;
              }
    ...
    toggleEditMode(router: Router, route: RouteLocationNormalizedLoaded) {
      ...
      if (networkStore.online) {
        ...
        toastStore.error({
          ...
          detail: "Sie haben keine Verbindung zum Netzwerk, bearbeiten nicht möglich!",
        });
      }
    },
