...
export const useGroupStore = defineStore("groupStore", () => {
  ...
  const fetchOnlyItem = async (id: string | RouteParamValue[]) => {
    try {
      ...
      item.value.updatedAt = data.group.updatedAt ? new Date(data.group.updatedAt) : null;
    } catch (err) {
  ...
  // ✅ UPDATE
  const { mutate: updateMutateGroup, loading: updatingLoading } =
    useMutation(MUTATIONEN_UPDATE_GROUP);
  ...
  // ✅ DELETE
  const { mutate: deleteMutateGroup, loading: deletingLoading } =
    useMutation(MUTATIONEN_DELETE_GROUP);
   ...
   */
  const routeToNewItem = (tabId?: string, groupId?: string): RouteLocationRaw => {
    return {
  ...
  const groupsForSelectedTab = computed(() => {
    ...
    return allGroups.value.filter((group) => group.tabId === selectedTabId.value);
  });
