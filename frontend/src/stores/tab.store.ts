...
import { type CustomColumnProps, type Group, type Tab } from "@/utils/interfaces.ts";
...
export const useTabStore = defineStore("tabStore", () => {
  ...
  const multiSortMeta: Ref<DataTableSortMeta[]> = ref([{ field: "order", order: -1 }]);
  ...
  const fetchOnlyItem = async (id: string | RouteParamValue[]) => {
    try {
      ...
      item.value.updatedAt = data.tab.updatedAt ? new Date(data.tab.updatedAt) : undefined;
    } catch (err) {
  ...
  // ✅ CREATE
  const { mutate: createMutateTab, loading: creatingLoading } = useMutation(MUTATIONEN_CREATE_TAB, {
    update(cache, { data }) {
  ...
  // ✅ UPDATE
  const { mutate: updateMutateTab, loading: updatingLoading } = useMutation(MUTATIONEN_UPDATE_TAB);
  ...
  // ✅ DELETE
  const { mutate: deleteMutateTab, loading: deletingLoading } = useMutation(MUTATIONEN_DELETE_TAB);
