...
export const useTagStore = defineStore("tagStore", () => {
  ...
  const fetchOnlyItem = async (id: string | RouteParamValue[]) => {
    try {
      ...
      item.value.updatedAt = data.tag.updatedAt ? new Date(data.tag.updatedAt) : null;
    } catch (err) {
  ...
  // ✅ CREATE
  const { mutate: createMutateTag, loading: creatingLoading } = useMutation(MUTATIONEN_CREATE_TAG, {
    update(cache, { data }) {
  ...
  // ✅ UPDATE
  const { mutate: updateMutateTag, loading: updatingLoading } = useMutation(MUTATIONEN_UPDATE_TAG);
  ...
  // ✅ DELETE
  const { mutate: deleteMutateTag, loading: deletingLoading } = useMutation(MUTATIONEN_DELETE_TAG);
