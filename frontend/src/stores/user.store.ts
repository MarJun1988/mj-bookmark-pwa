...
export const useUserStore = defineStore("userStore", () => {
  ...
  const fetchOnlyItem = async (id: string | RouteParamValue[]) => {
    try {
      ...
      item.value.updatedAt = data.user.updatedAt
  ...
  // ✅ UPDATE
  const { mutate: updateMutateUser, loading: updatingLoading } = useMutation(
    MUTATIONEN_UPDATE_USER,
  );
  ...
  const { mutate: updateMutateUserPassword, loading: updatingPasswordLoading } =
    useMutation(MUTATIONEN_UPDATE_USER_PASSWORD);
  ...
  // ✅ DELETE
  const { mutate: deleteMutateUser, loading: deletingLoading } = useMutation(
    MUTATIONEN_DELETE_USER,
  );
