...
export const useVersionStore = defineStore("versionStore", () => {
  ...
  const fetchOnlyItem = async (id: string | RouteParamValue[]) => {
    try {
      ...
      item.value.updatedAt = data.version.updatedAt
        ? new Date(data.version.updatedAt)
        : undefined;
    } catch (err) {
  ...
  const subscriptions: AnySubscriptionVersion[] = [
    ...
    },
    {
      ...
      handler: (msg: Version) => {
        ...
        pagedItems.value = pagedItems.value.map((a) =>
          a.id === msg.id ? msg : a,
        );
    ...
    },
    {
      ...
      handler: async (msg: SubscriptionDeleteVersion) => {
        ...
        const deletedVersions = Array.isArray(msg.version)
          ? msg.version
          : [msg.version];
  ...
  // ✅ UPDATE
  const { mutate: updateMutateVersion, loading: updatingLoading } = useMutation(
    MUTATIONEN_UPDATE_VERSION,
  );
  ...
  // ✅ DELETE
  const { mutate: deleteMutateVersion, loading: deletingLoading } = useMutation(
    MUTATIONEN_DELETE_VERSION,
  );
