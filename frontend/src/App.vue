...
<template>
  ...
  <DevBar v-if="isDev" />
  <OfflineBar />
  <div v-if="auth.isAuthenticated" class="min-h-screen flex flex-col bg-(--p-surface-ground)">
    <!-- TopBar -->
    ...
    <main class="flex-1 flex flex-col overflow-hidden m-auto w-11/12">
      ...
      >
        <template #content>
          <div v-if="!auth.isReady" class="p-6 text-center bg-[var(--p-surface-section">
            Loading…
          </div>
    ...
    <AppFooter :show-project-name="true" width-class="w-11/12" />
  </div>

  <router-view v-if="!auth.isAuthenticated" name="loginView" />
</template>
