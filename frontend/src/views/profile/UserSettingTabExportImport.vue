...
// GraphQL Mutation
const { mutate: importBookmarks, loading: importLoading } =
    useMutation(IMPORT_BOOKMARKS);
...
const exportData = async () => {
    ...
    try {
        ...
        link.href = url;
        link.download = `bookmarks-export-${new Date()
            .toISOString()
            .split("T")[0]}.json`;
        link.click();
...
<template>
  <div class="space-y-6">
    ...
    <p class="text-center text-muted-color">
      Hier kannst du deine Tabs, Gruppen und Einträge exportieren oder aus
      einer JSON-Datei importieren.
    </p>

    <!-- IMPORT MODE -->
    <div class="border rounded-lg p-4">
      <p class="font-semibold mb-3 text-center">
        Für den Import auswählen:
      </p>
    ...
    <div class="border rounded-lg p-4">
      ...
      <ul class="list-disc ml-6 space-y-1">
        <li><b>{{ stats.tabs }}</b> Tabs</li>
        <li><b>{{ stats.groups }}</b> Gruppen</li>
        <li><b>{{ stats.items }}</b> Einträge</li>
      </ul>
    </div>
    ...
    >
      ...
      >
        ...
        <ul class="list-disc ml-6 space-y-1">
          ...
          <li>
            ⚠ <b>{{ importResult.skippedItems }}</b> übersprungen
            (Duplikate)
          </li>
          <li>
