...
const activeTabId = computed({
  get() {
    ...
    return dashboardStore.activeTabId ?? dashboardStore.tabsForView[0]?.id ?? "";
...
const activeGroup: ComputedRef<Group | null> = computed<Group | null>(() => {
  ...
  return groupsForActiveTab.value.find((g) => g.id === activeGroupId.value) ?? null;
});

const itemsForActiveTab = computed(() => groupsForActiveTab.value.flatMap((g) => g.items ?? []));
...
const itemsForActiveGroup: ComputedRef<Item[]> = computed<Item[]>(() => {
  ...
  return itemsForActiveTab.value.filter((item) => item.groupId === activeGroupId.value);
});
const tabMenu = ref();
...
<template>
  <!-- Tabs Leiste -->
  <Tabs v-model:value="activeTabId" class="w-full" lazy>
    ...
    <TabPanels class="bg-(--p-surface-section)">
      ...
      >
        ...
        <TabPanelGroups
          ...
          @open-action-menu="openTabMenu"
          @add-item="router.push(groupStore.routeToNewItem(tab.id))"
        />
        ...
        <GroupContext
          ...
          :activeTab="activeTab"
          @add-item="router.push(itemStore.routeToNewItem(activeTabId, activeGroupId))"
        />
  ...
  <Menu ref="tabMenu" :model="menuItems" popup>
