...
// Item laden, wenn ID in Route (EDIT) - oder leeres Formular für CREATE
watch(
  () => route.params.id,
  async (id) => {
...
// Neues Item anlegen mit TabId aus Route (z.B. von Tab aus "Gruppe hinzufügen" Button)
watch(
  () => route.query.tabId,
  async (tabId) => {
    // 🔥 sicherstellen, dass Lookup-Daten da sind
    ...
    if (tabId) {
      formModel.value.__tabId = tabId as string;
    } else {
      formModel.value = { ...defaultItem };
    }
    isReady.value = true;
...
// Neues Item anlegen mit GroupId aus Route (z.B. von Gruppe aus "Gruppe hinzufügen" Button)
watch(
  () => route.query.groupId,
  async (groupId) => {
    // 🔥 sicherstellen, dass Lookup-Daten da sind
    ...
    if (groupId) {
      console.log("groupId aus Route:", groupId);

      formModel.value.groupId = groupId as string;
    } else {
      formModel.value = { ...defaultItem };
    }
    isReady.value = true;
...
onBeforeMount(async () => {
  ...
  await Promise.all([groupStore.fetchAllItems()]);
