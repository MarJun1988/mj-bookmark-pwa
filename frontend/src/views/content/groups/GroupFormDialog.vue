...
// Gruppe laden, wenn ID in Route (EDIT) - oder leeres Formular für CREATE
watch(
  () => route.params.id,
  async (id) => {
...
// Neue Gruppe anlegen mit TabId aus Route (z.B. von Tab aus "Gruppe hinzufügen" Button)
watch(
  () => route.query.tabId,
  async (tabId) => {
    // 🔥 sicherstellen, dass Lookup-Daten da sind
    await Promise.all([tabStore.fetchAllItemsForSelect()]);
    if (tabId) {
      formModel.value.tabId = tabId as string;
    } else {
      formModel.value = { ...defaultGroup };
    }
    isReady.value = true;
...
<template>
  ...
  >
    ...
    >
      ...
      <p class="text-base">
        Alle Bookmarks, Widgets und Notizen in dieser Gruppe werden ebenfalls gelöscht. <br />
        Diese Aktion kann nicht rückgängig gemacht werden! <br />
      </p>
      <p class="text-sm text-gray-500 mt-3">({{ itemCount }} Einträge)</p>
    </Message>
