...
const messageSeverity = ref<
  HintedString<"success" | "info" | "warn" | "error" | "secondary" | "contrast"> | undefined
>("success");
...
const isEmailValid = computed(() => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(registerUser.value.email));
    ...
    >
      <template #content>
        ...
        <div class="text-center space-y-1">
          ...
          <p class="text-sm text-surface-500">Erstelle jetzt dein neues Konto</p>
        </div>
        ...
        <div class="flex justify-end col-span-2">
          <Button label="Bereits ein Konto? Anmelden" link @click="router.push('/login')" />
        </div>
