...
const itemsForView = computed<ItemView[]>(() => {
  return dashboardStore.itemsForView(
    props.activeTab?.id,
    props.activeGroupId ?? undefined,
  );
});
...
<template>
  <section>
    ...
    >
      <template
        v-for="item in sortedItemsForView"
        :key="item.id"
      >
        ...
        <ItemCard
          ...
          :item="item"
          @open-action-menu="
            !item.__skeleton && emits('openActionMenu', $event, item as Item)
          "
        />
      </template>
