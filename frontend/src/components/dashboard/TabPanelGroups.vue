...
watch(
  () => props.groups,
  (groups) => {
    localGroups.value = [...groups].sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
  },
  { immediate: true },
);
...
const activeGroup = computed(() => {
  ...
  return props.groups.find((g) => g.id === dashboardStore.activeGroupId) ?? null;
});
...
<template>
  <div class="mb-4 flex flex-wrap items-center gap-2 sm:flex-nowrap sm:overflow-x-auto">
    <template v-if="mode === 'edit'">
      <VueDraggable
        ...
        @update="onUpdate"
      >
        <template v-for="group in localGroups" :key="group.id">
          <div class="flex items-center gap-1">
            <Button
              :severity="group.id === dashboardStore.activeGroupId ? 'primary' : 'secondary'"
              class="shrink-0 px-4 py-2 text-sm rounded-lg transition whitespace-nowrap"
              rounded
              size="small"
              @click="selectGroup(group.id)"
            >
              <span class="line-clamp-2 wrap-break-word leading-tight text-center">
                {{ group.title }}
              </span>
            </Button>
    ...
    <template v-else>
      ...
      <div class="flex items-center gap-2">
        ...
        <!-- DESKTOP -->
        <div v-else class="flex gap-2 overflow-x-auto scrollbar-hide p-2">
          <!-- Alle -->
          <Button
            :severity="dashboardStore.activeGroupId === null ? 'primary' : 'secondary'"
            class="shrink-0 px-4 py-2 text-sm rounded-lg transition whitespace-nowrap"
            label="Alle"
            size="small"
          ...
          >
            <Skeleton v-if="group.__skeleton" class="min-w-16 min-h-4" />
            <span v-else>
              {{ group.title }}
            </span>
          </Button>
      ...
      >
        <div class="space-y-2">
          ...
          <Button
            ...
            @click="!group.__skeleton && selectGroup(group.id)"
          >
            <Skeleton v-if="group.__skeleton" height="1rem" width="6rem" />
            <span v-else>
              {{ group.title }}
            </span>
          </Button>
