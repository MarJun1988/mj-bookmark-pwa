...
const { pagedItems, totalCount, pageSize, columns, multiSortMeta } =
...
const rowsPerPageOptions: number[] = [
  1, 5, 10, 20, 25, 50, 75, 100, 500, 1000, 5000,
];
...
const selectedColumns: Ref<CustomColumnProps[]> = ref(
  columns.value.filter(
    (column: CustomColumnProps) => column?.defaultShowing === true,
  ),
);
...
const onToggle = (val: CustomColumnProps[]) => {
  selectedColumns.value = columns.value.filter((col: CustomColumnProps) =>
    val.includes(col),
  );
};
...
const globalFilter = computed({
  ...
  set: (val) =>
    ((filters.value["global"] as DataTableFilterMetaData).value = val),
});
    ...
    >
      ...
      <template #header>
        <div class="flex flex-wrap gap-2 justify-between">
          ...
          <div class="w-full sm:w-auto flex justify-center">
            <div v-if="!props.notShowActionButton.create">
              <Button
                v-tooltip.right="{
            ...
            <div v-if="!props.notShowActionButton.delete">
              <OverlayBadge
              ...
              >
                <Button
                  ...
                  :disabled="
                    !selectedItems || !selectedItems.length || isDisabled
                  "
                  icon="pi pi-trash"
                  severity="danger"
          ...
          <!-- Suche -->
          <div
            class="w-full sm:w-auto justify-items-end flex gap-4 mt-4 md:mt-0"
          >
            <FloatLabel :variant="ui.floatLabelVariant">
              <IconField>
                <InputIcon>
      ...
      >
        <div class="flex flex-wrap w-full gap-4 justify-between items-center">
          ...
          <div class="w-full sm:w-auto text-center">
            <span>
              {{ first }} bis {{ last }} von {{ totalRecords }} Einträgen
            </span>
          </div>
        </div>
      </template>
      ...
      >
        <template #body="slotProps">
          <Skeleton
            v-if="slotProps.data.__skeleton"
            class="min-h-9.20"
          />
          ...
          <template v-else-if="slotProps.field === 'tags.id'">
            <div class="flex flex-wrap gap-1 max-w-48">
              ...
              <Tag
                ...
                v-tooltip.top="
                  slotProps.data.tags.map((t: Tag) => t.name).join(', ')
                "
          ...
          <template v-else-if="col.withIcon">
            <div class="flex items-center gap-2 min-w-0">
              ...
              >
                <img
                  v-if="
                    col.options?.icon?.field &&
                      slotProps.data[col.options.icon.field]
                  "
        ...
        >
          <FloatLabel :variant="ui.floatLabelVariant">
            ...
            <MultiSelect
              v-if="
                col.dataType === 'multiselect' && col.filterKey !== undefined
              "
              v-model="filterModel.value"
      ...
      <Column
        v-if="
          !props.notShowActionButton.update && !props.notShowActionButton.delete
        "
        :exportable="false"
        :pt="{
          columnHeaderContent: {
        ...
        <template #body="slotProps">
          <div class="flex flex-wrap justify-between gap-2">
            ...
            <Button
              ...
              variant="outlined"
              @click="
                router.push(props.store.routeToDeleteItem(slotProps.data))
              "
            />
          </div>
        </template>
