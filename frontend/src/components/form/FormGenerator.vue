  ...
  (newModel, oldModel) => {
    ...
    props.fields.forEach((field) => {
      ...
      const depChangedByUser = field.dependsOn.some(
        (dep) => newModel[dep] !== oldModel?.[dep] && userChanged.value.has(dep),
      );
...
const isFormValid = computed(() => {
  return props.fields.every((field) => {
    ...
    // nicht required → egal
    const isRequired =
      typeof field.required === "function" ? field.required(props.modelValue) : field.required;
...
<template>
  <form class="grid grid-cols-12 gap-4" @submit.prevent="emit('submit')">
    <p class="col-span-12 text-sm text-muted"><span class="text-red-500">*</span> Pflichtfelder</p>
    <FieldRenderer
    ...
    <button :aria-disabled="!isFormValid" :disabled="!isFormValid" type="submit" class="hidden" />
  </form>
</template>
